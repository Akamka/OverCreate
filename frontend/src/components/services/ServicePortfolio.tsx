...
import React, {
  useMemo, useRef, useState, useEffect, useCallback, type CSSProperties,
} from 'react';
...
/* ---------- единственный допустимый источник превью ---------- */
function getCoverSrc(item: ServicePortfolioItem): string | null {
  const raw = item.cover_url?.trim();
  if (!raw) return null;

  const abs = toMediaUrl(raw) ?? null;
  if (!abs) return null;

  // БЛОКИРУЕМ старый эндпоинт, который даёт 404
  if (/\/api\/media\/portfolio\/\d+\/cover/i.test(abs)) {
    console.warn('[ServicePortfolio] blocked legacy cover url:', abs, 'for item:', item.id);
    return null;
  }
  return abs;
}
...
export default function ServicePortfolio({
  ...
  const rAFScrollTo = useCallback((left: number, duration = 560) => {
    ...
    const d = left - start;
    if (Math.abs(d) < 1) { vp.scrollLeft = left; return; }
    const t0 = performance.now();
    const step = (now: number) => {
      const p = Math.min(1, (now - t0) / duration);
      vp.scrollLeft = start + d * ease(p);
  ...
  const getVpPadLeft = (): number => {
    const vp = viewportRef.current; if (!vp) return 0;
    const cs = getComputedStyle(vp); const val = parseFloat(cs.paddingLeft || '0');
  ...
  const recalcCardWidth = useCallback(() => {
    const pageW = getPageWidth(); if (pageW <= 0) return;
  ...
  useEffect(() => { recalcCardWidth(); }, [recalcCardWidth]);

  useEffect(() => {
    const vp = viewportRef.current; const track = trackRef.current;
    if (!vp || !track) return;
    let startX = 0, startLeft = 0, dragging = false, lastX = 0, lastT = 0, velocity = 0;
    const isInteractive = (el: EventTarget | null): boolean =>
      el instanceof Element && Boolean(el.closest('button,a,[role="button"],input,select,textarea,label'));

    const down = (e: PointerEvent) => {
      if (isInteractive(e.target)) return;
      dragging = true; startX = e.clientX; startLeft = vp.scrollLeft; lastX = e.clientX; lastT = e.timeStamp;
      track.setPointerCapture(e.pointerId); (track.style as CSSStyleDeclaration).cursor = 'grabbing'; cancelAnim();
    };
    const move = (e: PointerEvent) => {
      if (!dragging) return;
      const dx = e.clientX - startX; vp.scrollLeft = startLeft - dx;
      const dt = e.timeStamp - lastT || 1; velocity = (e.clientX - lastX) / dt; lastX = e.clientX; lastT = e.timeStamp;
    };
    const up = () => {
      if (!dragging) return; dragging = false; track.style.removeProperty('cursor');
      const positions = getPagePositions(); let curr = 0; let best = Number.POSITIVE_INFINITY;
      positions.forEach((pos, i) => { const d = Math.abs(vp.scrollLeft - pos); if (d < best) { best = d; curr = i; } });
      const threshold = 0.25; const target = velocity > threshold ? curr - 1 : velocity < -threshold ? curr + 1 : curr;
      goTo(target);
    };
    ...
    <section id="portfolio" className="oc-section section-soft" style={accVars as CSSProperties}>
      <div className="mx-auto max-w-[1200px] px-4 md:px-6">
        <div className="mb-6 md:mb-8">
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-white/90">{title}</h2>
        ...
        <div className="relative" style={{
          ...
          {!controlsDisabled && (
            <>
              <button aria-label="Prev" onClick={prev}
                ...
                style={{ position: 'absolute', top: '50%', left: `-${NAV_SIZE + NAV_OUT}px`, width: NAV_SIZE, height: NAV_SIZE, transform: 'translateY(-50%)', zIndex: 20 }}>
                <span className="text-2xl leading-none"
                  style={{ backgroundImage: 'linear-gradient(135deg, rgb(var(--acc1)), rgb(var(--acc2)))', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>‹</span>
              </button>
              <button aria-label="Next" onClick={next}
                className="group hidden md:grid place-items-center rounded-full border border-white/10 bg-black/10 text-white/80 shadow-lg backdrop-blur-sm transition will-change-transform hover:scale-105 active:scale-95"
                style={{ position: 'absolute', top: '50%', right: `-${NAV_SIZE + NAV_OUT}px`, width: NAV_SIZE, height: NAV_SIZE, transform: 'translateY(-50%)', zIndex: 20 }}>
                <span className="text-2xl leading-none"
                  style={{ backgroundImage: 'linear-gradient(135deg, rgb(var(--acc1)), rgb(var(--acc2)))', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>›</span>
              </button>
            </>
          ...
          <div ref={viewportRef} className="no-scrollbar pf-viewport" data-lenis-prevent-touch=""
            style={{ paddingLeft: 'var(--pf-bleed)', paddingRight: 'var(--pf-bleed)', overflowX: 'auto', overflowY: 'visible', scrollBehavior: 'smooth' }}>
            <div ref={trackRef} className="pf-track flex select-none touch-pan-x" style={{ gap: GAP }}>
              {items.map((it) => (
      ...
      {openedId != null && (
        <PortfolioModal id={openedId} onClose={() => setOpenedId(null)} accFrom={accentFrom} accTo={accentTo} />
      )}
    </section>
  );
}

function Dots({ pages, page, goTo }: { pages: number; page: number; goTo: (i: number) => void; }) {
  return (
    <div className="mt-5 flex items-center justify-center gap-2 select-none">
      {Array.from({ length: pages }).map((_, i) => (
        <button key={i} aria-label={`Go to page ${i + 1}`} onClick={() => goTo(i)}
          className="h-2 rounded-full transition"
          style={{ width: i === page ? 22 : 8, background: i === page ? 'rgb(var(--acc2))' : 'rgba(255,255,255,.25)', opacity: i === page ? 1 : 0.7 }} />
      ))}
    </div>
  );
}

function Card({ item, cardW, onOpen }: { item: ServicePortfolioItem; cardW: number; onOpen: (id: number) => void; }) {
  ...
  const numericId: number | null =
    ...
    typeof item.id === 'string' ? Number.parseInt(item.id, 10) || null : null;
  ...
  return (
    <div className="pf-card"
      ...
      style={{ width: `${cardW}px`, flex: `0 0 ${cardW}px`, transform: `perspective(900px) rotateX(${tilt.rx}deg) rotateY(${tilt.ry}deg)`, zIndex: hover ? 20 : 1, position: 'relative', willChange: 'transform' } as CSSProperties}>
        ...
        {src ? (
          imgFallback ? (
            ...
            <Image
              ...
              style={{ objectFit: fit }}
              className="pf-img"
              onError={() => { console.warn('[ServicePortfolio cover] failed:', src); setImgFallback(true); }}
            />
          )
        ) : (
