import type { Paginated, PortfolioItem, ServiceSlug } from "../types";

/* ---------------- базовые конфиги из env ----------------
   Поддерживаем оба варианта имён:
   - BASE: VITE_API_BASE_URL ИЛИ VITE_API_BASE
   - TOKEN: VITE_ADMIN_TOKEN ИЛИ VITE_DEFAULT_ADMIN_TOKEN
--------------------------------------------------------- */
...
export async function adminUpdatePortfolio(
  ...
  if (patch.gallery_files && Array.isArray(patch.gallery_files)) {
    patch.gallery_files.forEach((f) => fd.append("gallery_files[]", f));
  }
  if (patch.gallery && Array.isArray(patch.gallery)) {
